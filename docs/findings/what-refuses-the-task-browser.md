...
- **Cross-device hybrid, the QR sheet.** Not reachable, and the reason is in Electron's source rather than in what its docs omit. `ElectronAuthenticatorRequestClientDelegate::RegisterActionCallbacks` is handed nine callbacks by Chromium, two of them the hybrid path's own -- `bluetooth_adapter_power_on_callback` and `request_ble_permission_callback` -- and keeps only `cancel_callback_`. The delegate overrides five methods in total: relying-party id, start and stop observing, those callbacks, and `SelectAccount`. It never configures a caBLE discovery, so no QR secret is generated and no hybrid transport is offered, whatever the page asks for.
  
  Chromium's caBLE implementation is compiled in and unused. What is missing is the embedder wiring and the UI, which in Chrome is `chrome/browser/ui/views/webauthn/authenticator_qr_centered_view.h` -- one of 994 `chrome/browser/ui/` references in a shipping fork's binary against Electron's 16. An app that has this got it by shipping Chrome's browser layer, not by configuring Electron better.
  
  Worth knowing before anyone re-opens this: the delegate's copyright header is 2026, so the whole surface is new and narrow rather than settled, and an upstream ask is not obviously hopeless. Also worth knowing what does not work -- Apple's `AuthenticationServices` does support cross-device QR, but it requires a declared associated domain per relying party, which is coherent for an app authenticating to its own service and impossible for a browser serving arbitrary sites. Closing it means an upstream Electron change, or a native `AuthenticationServices` bridge whose page-side shim over `navigator.credentials` is the one thing this file says not to write — on a sign-in page, where it would be read.
