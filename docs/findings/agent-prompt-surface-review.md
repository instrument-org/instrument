...
**The system prompt cannot.** `agent.getMessages({ sessionId, taskId })` (`agents/types.ts`) is not given the model, and `prepare-model-messages.ts` rebuilds the session-context message only when it is older than `STALE_MESSAGE_THRESHOLD_MINUTES` (60) -- never when the model changes. `model` *is* in scope at the call site, so threading it is trivial; the staleness rule is the real obstacle. A model-conditional system prompt built on today's rebuild logic would keep serving Opus's trimmed prompt for up to an hour after the user switched to Haiku.

So the shape of any tiering work is: thread `model` into `getMessages`, **and** make the rebuild check compare the model that produced the stored context against the model about to run. This is the same reasoning already recorded in the `browserTargetingGuidance` comment, which chose *not* to state volatile facts in session context for exactly this reason.
