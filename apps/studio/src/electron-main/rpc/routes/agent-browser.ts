...
const live = {
  ...
  // its guests to this set (mount on add, dispose on remove), replacing the old
  // imperative mount/unmount IPC. Re-subscribing always yields the current set,
  // so a guest can't be stranded by a command sent before the listener existed.
