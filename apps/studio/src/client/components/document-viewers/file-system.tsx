...
import {
  ...
  CommandItem,
  CommandList,
} from "@/client/components/ui/extend/command";
import {
  Dialog,
  DialogClose,
  DialogContent,
  ...
  DialogPanel,
  DialogTitle,
} from "@/client/components/ui/extend/dialog";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import {
  ScrollArea,
  ScrollAreaPrimitive,
} from "@/client/components/ui/extend/scroll-area";
import {
  Select,
  SelectContent,
  SelectItem,
...
export type FileSystemProps = {
  ...
  /**
   * Called instead of `window.open` when an opened file has no built-in viewer.
   * Hosts that cannot open a browser tab (or want their own handling) supply
   * this; without it the resolved URL opens in a new tab.
   */
  onUnsupportedFileOpen?: (
    file: FileSystemFileItem,
    url: null | string,
  ) => void;
...
function fileMatchesFilter(file: FileEntry, filter: FileSystemFilter) {
  ...
  // Guarded by the `filter.value.length === 0` check above.
...
function FileGenericPreview({ file }: { file: FileEntry }) {
  ...
  return (
    <div
      className="flex size-full flex-col items-center justify-center gap-1.5 bg-white text-gray-400 dark:bg-gray-100"
...
function FileVisual({
  ...
  const thumbnail = (
    <FileThumbnail
      ...
      previewClassName={cn("bg-white dark:bg-gray-100", previewClassName)}
      previewContent={
  ...
  const openFile = React.useCallback(
    (file: FileEntry) => {
      void (async () => {
        ...
        } else if (onUnsupportedFileOpen) {
          onUnsupportedFileOpen(file, url);
      ...
      <div className="relative grid h-12 shrink-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 border-b bg-muted/40 px-2">
        <div className="flex min-w-0 items-center gap-0.5">
          <button
            ...
            type="button"
          >
            <HugeiconsIcon className="size-4.5" icon={ArrowLeft01Icon} />
          </button>
          <button
            aria-label="Forward"
            className="flex size-7 shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors outline-none hover:bg-accent hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-40"
            ...
            type="button"
          >
            <HugeiconsIcon className="size-4.5" icon={ArrowRight01Icon} />
          </button>
  ...
  return (
    ...
    >
      ...
      <CommandList className="max-h-none">
        ...
        <ScrollArea className="h-auto max-h-64" orientation="vertical">
      ...
      <DropdownMenuContent align="end" className="min-w-44">
        ...
        {(["dateModified", "dateCreated"] as const).map((type) => (
          <DropdownMenuSub key={type}>
            ...
            <DropdownMenuSubContent>
              <ScrollArea className="h-auto max-h-72" orientation="vertical">
...
function moveGridSelection({
  ...
  if (currentIndex === -1) {
    ...
    const currentElement = currentEntry
      ? itemRefs.get(currentEntry.path)
      : undefined;
