...
Cmd+R means the page in front of the user. The foreground browser panel claims the chord through [foreground-browser-registry.ts](../../apps/studio/src/client/lib/foreground-browser-registry.ts), the same way it claims Cmd+F, and with no panel claiming it the chord does nothing at all outside developer mode ([use-app-commands.ts](../../apps/studio/src/client/hooks/use-app-commands.ts)). The app reload survives as `reloadApp` (Cmd+Shift+R) in the Developer group, whose chords are bound only in developer mode, and as the button [app-error-fallback.tsx](../../apps/studio/src/client/components/app-error-fallback.tsx) puts on screen when the shell crashes — which is the salvage path the chord was being kept for, and the one that is actually discoverable in the situation it is meant for.
