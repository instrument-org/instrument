...
22. **The clamp was the expensive defect, not the refusal.** Five successful reads asked for a rectangle outside the view and were quietly trimmed to fit: one asked for the left half in the file's pixel space and was handed the full width, another asked for a square and got a letterbox. Each came back as a real magnified picture of somewhere the model did not ask about, described by a sentence naming the rectangle used -- which reads as confirmation. This is the same "a magnified crop is believed" lesson as the 1x1 flat colour, and it is worse, because the erroring shapes cost a round trip while this one costs an answer. A clamped read now leads with the rectangle asked for and the space it overran.
