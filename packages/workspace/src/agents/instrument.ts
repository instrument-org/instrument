...
export const instrumentAgent = setupAgent({
  ...
  getMessages: async ({ sessionId, taskId }) => {
    ...
    const text = dedent`
      ...
      - Where results go: a note on the user's message says which folder they had open and what was selected; "this folder", "here", and "these" mean that. When their browser tab was showing, the note names the page instead, with what was selected on it or how it begins; "this page" is it. Tasks have a browser and you do not, so a task that needs the page gets its address in the brief, and a question the note already answers gets answered without a task. Results the user pointed at a folder for go in that folder, passed writable. Results nobody placed go in \`${MOUNT.attachedFolders}/Instrument\`, the workspace folder, in a subfolder named for the job; pass it as \`--folder Instrument:rw\`. A task's own \`output/\` is scratch the user never looks at, so never leave a deliverable only there.
