...
Status: active — design settled, not started.
...
(status open) names this approach and its blocker; [in-app-browser.md](../../architecture/in-app-browser.md)
is where the guest actually lives.

## Evidence status — read this before trusting the answers below

The brief asked for the three open questions to be answered against a running
Studio. **They were not.** This environment could not install dependencies: the
root devDependency `@instrument-org/agent-hooks` resolves to a private
`codeload.github.com` tarball that pnpm cannot authenticate (403), and every
route around it — a local `link:`, a scoped `--filter` install, a user-level
npmrc auth entry — was refused by the sandbox. With no `node_modules` there is
no Studio to boot, no `tsgo`, and no lint.

The answers below are therefore **grounded in code, not observed**. Each one
names the file and line of reasoning it rests on, and
[Verification](#verification-what-has-to-be-driven-in-a-running-studio) gives the
procedure that would confirm or refute it. Two of them (Q1's modal case, and the
Radix pointer-events interaction) are genuinely uncertain and are called out as
such. Nothing here should be built on before that pass runs.
...
overlay is now covering. **That is a bug that already exists on `main`** (open the
expand modal over a live browser panel and the guest paints through the overlay),
and this plan has to fix it rather than inherit it. See
...
return (`{kind: "session", id, sessionId}` | `{kind: "artifact", id}`), which
makes every existing consumer that wants `sessionId` a compile error until it has
decided what to do about the artifact case — which is the point.
...
- **`dispatch-command.ts`** is reached only over CDP from `agent-browser`, which
  ...
  parseable `BrowserTargetId`. **Both should exclude artifact-kind targets**, so
...
outside-dismiss. **This is the item most likely to change the shape of the modal
work, and it cannot be settled by reading.**
...
## Verification: what has to be driven in a running Studio

Per `.agents/skills/validate-changes/SKILL.md` this is rung 4 throughout —
guests, focus, visibility and z-order are not observable from reading. **None of
this has run.** In rough dependency order:

1. **Q1, directly.** Open a task's browser panel, then open an HTML artifact.
   Confirm they are the same slot and the browser panel is replaced. Then open
   the expand modal over a live browser panel on `main` and confirm the guest
   paints through the overlay — establishing the pre-existing bug before fixing
   it.
2. **Q2, directly.** Give an artifact target a distinct `partitionDir`, write
   `localStorage` from an artifact page, and confirm the agent's browser guest on
   the same origin cannot see it. Confirm the reverse too.
3. **The modal, all three sub-problems.** Guest paints above the overlay; guest
   accepts clicks with the dialog open (this is the one expected to fail first);
   panel guest parks when the modal opens.
4. **Sibling fetch from a real origin.** An agent-authored page that
   `fetch`es `./data.json`, and one that loads a sibling `<script>` and
   `<img>`. Both under the version query string the asset URL carries.
5. **Range requests.** A large media file referenced from artifact HTML, seeking
   mid-file.
6. **App zoom at 0.5x and 2x**, in the panel and the modal, plus a splitter drag
   at each — the artifact panel slides in via a transform, which is why
   `use-browser-slot` tracks per-frame until the position holds.
7. **The nonce replacement.** Click a link inside an artifact, then: reload
   returns to the sub-page; escape-to-root returns to the entry page; back
   works; re-selecting the file in the sidebar returns to the entry page.
8. **Leak checks.** Close the panel and confirm the target is gone after the
   grace period (`debug-snapshot.ts` shows live entries). Close the task's tab
   mid-preview. Force-quit the renderer. Switch files rapidly and confirm no
   create/destroy thrash.
9. **Agent surface unchanged.** `agent-browser` in a task with an artifact
   preview open: confirm `/json?id=` does not list the artifact target and the
   agent's own target selection is unaffected.
10. **`viewMode: "raw"`** on the same file, before and after.
