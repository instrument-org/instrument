...
type Tool = RPCOutput["apps"]["inspect"][number];
...
export function AppInspector({
  ...
  const isBare = (tool: Tool) => tool.params.every((p) => !p.required);
 ...
 */
function splitCode(text: string): { code: string; prose: string } {
...
function languageOf(code: string): string | undefined {
  if (/^\s*[[{]/.test(code)) {
    return "json";
  }
  if (
 ...
 */
function isListAnswer(value: unknown): boolean {
 ...
 */
function childrenOf(value: unknown): [string, unknown][] | undefined {
  if (Array.isArray(value)) {
    if (
      value.length === 0 ||
...
function labelOf(tool: Tool): string {
  if (tool.title) {
    return tool.title;
  }
...
function listsFirst(a: Tool, b: Tool): number {
 ...
 * not that.
 */
function parseSequence(text: string): unknown[] | undefined {
  ...
  for (let index = 0; index < text.length; index++) {
    ...
    } else if (depth === 0 && char !== undefined && !/\s/.test(char)) {
      return undefined;
    }
  }
 ...
 */
function recordsOf(value: unknown): Json[] {
  if (Array.isArray(value)) {
...
function titleOf(record: Json): string {
    ...
    const byName =
      typeof own === "string"
        ...
        : /name$/i.test(param.name) && typeof record.name === "string"
          ...
          : undefined;
    const byId =
      /(?:^|_|[a-z])(?:id|uuid)$/i.test(param.name) ||
