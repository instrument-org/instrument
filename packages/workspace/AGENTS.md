...
- `session-context` message (system prompt + `agent.getMessages`) is the session's immutable baseline: written once by `prepare-model-messages.ts` when the session first needs model input, then reused byte for byte. Never rebuilt, so the request prefix a provider cache is keyed on does not move.
