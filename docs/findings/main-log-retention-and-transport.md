...
**A throw followed by a quit needs a write that outlives it.** An uncaught exception does not end the main process: Electron's own `uncaughtException` listener never exits, and shows an error dialog only while no other listener is attached, which the same module's listener makes sure of. But a throw in main is the one most likely to be followed by a quit or a force-quit, and a line handed to the queue just before one is a line nobody ever reads. So an `uncaughtExceptionMonitor` also appends that record synchronously to `last-crash.log` in `userData`, and it is folded into this file on the next start. The exposure that remains is a native main-process crash, and a JavaScript handler is not the instrument for that: it runs none of its own code, which is the same reason those crashes need minidumps rather than a log line.
