...
- **codex-app** (`~/code/reference/codex-app`): model emits a normal Markdown link; a custom `<a>`
...
- **craft-agents-oss** (`~/code/reference/craft-agents-oss`): plain styled `<a>`; a
...
local file. We do the same. A codex-style explicit token is a possible later add, not needed now.

## Design decisions

- **Rendering: file chip** (icon + filename), matching the app's existing file cards/rows and
  codex-app. Not a plain underlined link.
...
- Only if we later want a chip without a visible path or with extra metadata (line ranges, custom
  label without a real link). Model on codex-app's `【path†L..】` token + a remark tokenizer. Likely
...
4. Middle/cmd-click: ensure the chip's DOM `href` can't escape into the OS browser (mirror
   craft-agents-oss's sanitized-`href` guard).
