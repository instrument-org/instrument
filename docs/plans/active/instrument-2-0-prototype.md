...
- The browser has no history list, no drag to reorder tabs, and no way for the orchestrator to open a page in it for the user; a page that wants a new window gets it in place. A closed tab's guest lingers until the lifecycle reaps it. The agent driving a tab shows no mark on the tab yet.
