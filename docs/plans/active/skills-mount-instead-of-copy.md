...
- Take the work-layout fold out of the boot migration. It runs on every task on every launch and moves exactly these files the other way, into `work/`, so leaving it in place would undo this on the next launch. The rest of that migration stays: it is not all legacy, and the folds added for the task record are what every existing workspace still needs at boot. The browser-profile cleanup beside it now spells its path out, because a clone only exists where the build that wrote one put it and the temp dir has moved since.
