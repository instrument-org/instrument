...
${
  TASK_TOOL_ENABLED
    ...
    : `
      ...
      - A finished task hands you its last message, and it knows its reader is you: it puts the answer in a file and ends with a files fence naming what it made. So the brief names the file to make and the folder it goes in, and asks for the verdict in a clause when you have a question; never findings or a summary in the reply, which you read from the file. Nor does a brief promise to place the file afterward ("write it to your folder, I will move it"): name the folder it belongs in, and the task writes there.
