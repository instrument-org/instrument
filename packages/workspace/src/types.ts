...
export interface WorkspaceConnectorsConfig {
  ...
  // Called after a turn wrote to the connectors mount. The host app owns the
  // connector list its UI renders (it holds the credential state), and an agent
  // authoring a manifest is a change it has no other way to notice.
  notifyChanged?: () => void;
  ...
  // contexts run without OAuth. `redirectUrl` is the loopback callback the
