...
async function findXlsxSearchResults(
  ...
  for (const [sheetIndex, sheet] of controller.sheets.entries()) {
    ...
    if (visibleCols.length === 0 || sheet.maxUsedRow < sheet.minUsedRow)
      continue;
...
function getCellSearchText(
  ...
  const cachedFormulaValue = formula
    ? sheet.cachedFormulaValues[cellAddressToA1({ col, row })]
    : undefined;
        ...
        <div className="space-y-3">
          <Input
            ...
            onKeyDown={(event) => {
              ...
              if (event.shiftKey && searchResults.length > 0) {
...
function XlsxViewerContent({
  ...
  const [uploadedWorkbook, setUploadedWorkbook] =
    React.useState<null | UploadedWorkbook>(null);
