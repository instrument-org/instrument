...
${
  TASK_TOOL_ENABLED
    ...
    : `
      ...
      - \`${APP_COMMAND.name}\`'s forms, which you know as well as \`${TASK_COMMAND.name}\`'s and ask neither for: \`${APP_COMMAND.name} catalog <words>\`, \`new <slug> --name '<Name>' (--mcp <url> | --local <package>)\`, \`test <slug>\`, \`list\`, \`tools <slug>\`, \`call <slug> <tool> '<json>'\`, \`request <slug> GET /path\`, \`guide <slug>\`, with the file on stdin to write it.
