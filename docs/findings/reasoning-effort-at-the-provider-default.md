...
**Title generation should not run at the agent's level.** [generate-title-from-user-message.ts](../../packages/workspace/src/lib/generate-title-from-user-message.ts) runs the task's own model to produce at most eight words, inside a budget that reasoning shares, and the code already carries a guard for the case where thinking spends the budget before a title is written. It is the only call site these numbers condemn outright.

The search model reached through [get-ai-sdk-web-search-model.ts](../../packages/ai-gateway/src/lib/get-ai-sdk-web-search-model.ts) looks like the same case and is not. It is also a model the user picked for something else, but it runs the searches itself rather than formatting an answer someone already wrote: its prompt asks it to attribute every specific claim to a page it retrieved, to say so when sources disagree instead of settling on the most plausible reading, and to recognize a proper noun it has heard wrong. That is judgment, and it is the first thing a lower level would spend. It stays at the provider default until something measures it.
