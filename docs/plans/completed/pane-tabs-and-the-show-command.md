...
It is the imperative half of [presentation-syntax.md](./presentation-syntax.md). A ` ```files ` fence is how a reply hands files over and leaves a record; `show` is how it puts one on screen right now. The two are separate on purpose: closing the pane must not erase what the reply said it produced.
...
**Pane state goes in `state.json`, not the URL.** A positional tab list does not belong in a search param: it is too much data, drag-reorder would rewrite the URL on every drop, and nothing outside the app ever links to it. [task-state-store.ts](../../../packages/workspace/src/lib/task-state-store.ts) already holds exactly this kind of per-task UI state, and `promptDraft` is the precedent for writing it from the client on a debounce.
