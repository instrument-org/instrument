...
- A program that imports a package installed in the task's virtualenv runs natively under `python` as well, chosen before it runs by reading its import statements (script file, `-c` code, or a heredoc on stdin). It changes no containment, since `python-native` is one word away and sees only the task folder either way, and it lets a shipped skill say `python` the way any other agent's skill would. It is static and fails open: a dynamic import, or a package not installed yet, stays in the sandbox, whose error says to install and run again. This is a choice made before the run, not the fallback after a failed import rejected below.
