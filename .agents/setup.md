...
Before the README setup flow (`pnpm install`, `./scripts/setup.sh`, `pnpm run dev:studio`).
...
- `pnpm run dev:studio` (README) or VS Code/Cursor **Run and Debug → Studio** after setup.
- **Platform API** (auth, billing, models): run the separate `internal` API at **<http://localhost:49100>** so `MAIN_VITE_APP_API_BASE_URL` in `apps/studio/.env.local` resolves. The Electron shell can start without it; sign-in and gateway calls fail until the API is up.
