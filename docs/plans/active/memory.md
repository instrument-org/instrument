...
Cost is the thing to watch rather than safety: reading a memory store that size ran 160K to 310K tokens for one import. Reading it in the conversation also spends that against the thread's own window, which is why the folder now goes to a task, handed read-only, that reports the facts back for the conversation to save.

**What a real import found, 2026-09-19.** Run against an actual `~/.claude`, the mechanics held: never delegated, never wrote, and it listed `.credentials.json` without opening it, all three checked against the commands rather than its own account of them. The content was the problem. Of ten memories, three were not about the person at all but instructions written for a coding assistant: which ssh host is which laptop, never to sit in a polling loop, and never to `open` a rendered file because a viewer app watches that folder. That last one would have told this product not to use `open`, which the conversation prompt tells it to reach for constantly. So the filter is not "is this about code", which all ten passed. It is two questions: is this a fact about the person or an instruction to a different tool, and does it contradict something this product does by design. Both are in the prompt now.
