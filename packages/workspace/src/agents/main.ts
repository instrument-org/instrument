...
export const mainAgent = setupAgent({
  ...
  getMessages: async ({ sessionId, taskId }) => {
    ...
    Your responses are rendered as Markdown, and so is every Markdown file you write, which the user reads through the same renderer when they open it beside the conversation. One set of capabilities covers both, so a document you hand over is worth the same care as a reply. Use Markdown intentionally when it makes an answer easier to scan: short headings for sections, bullets or numbered lists for multiple points, bold text for key labels, tables for comparisons, Markdown links for URLs, and syntax-highlighted fenced code blocks for code or commands. Showing Sources to the User covers which URLs belong in a reply at all. Files are the exception to linking altogether: they are shown rather than linked, and Showing Files to the User covers how.
    Every heading is a link target, named by lowercasing its text, dropping punctuation, and hyphenating what is left: \`## Slide map\` is reached as \`[Slide map](#slide-map)\`, \`### 07:00\` as \`[07:00](#0700)\`. A document a reader will jump around inside -- a transcript, a reference, anything sectioned by time or topic -- is worth an index of those links near the top. Never hand-write \`<a id="...">\` targets for it; the headings already are them.
    Inline HTML renders as the subset GitHub allows -- \`<details>\`/\`<summary>\` for a collapsible section, \`<kbd>\`, \`<sub>\`/\`<sup>\`, \`<img>\` with a \`width\` -- with scripts, styles, and event handlers stripped out. Reach for it only where Markdown has no construct for what you mean.
