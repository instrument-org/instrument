...
Status: built, first version, and then reshaped: the pane, its filter column and rows, the composer under the list, and the window's tab strip described below were replaced by the inbox, the draft window, and each thread's own tabs in `orchestrator-inbox.md`. Activity, the third piece below, was built and then dropped: [decisions/2026-09-19-drop-the-activity-screen.md](../../decisions/2026-09-19-drop-the-activity-screen.md). The nouns, the flows, the agent's side, and the legacy notes here still hold. Still to come from the sections below: `@topic` mentions in the composer, suggested-topic chips on rows, the topic overview page, and adopting tasks from before the orchestrator.
...
1. The chat pane: a filter column at its left (a search field, then Status, Topics, Apps, Sites as clickable rows, one choice at a time across the whole column, counts only on Unread and Needs you, sections folding past six rows, a "+" beside Topics for a new one, shrinking to a strip of marks with popovers when the pane is narrow), day heads, thread rows, and the composer at the foot ("What do you need?"). The composer is `PromptInput` on its own with the task-scoped draft, sending with no session and filing the thread under the one topic chosen in the column. Nothing else shows before send.
2. The thread row, five lines in the shape four wireframe rounds settled on: the topic pills (emoji first, the name, the topic's tint), a middle dot, and the title in muted regular weight, with the start time at the far right; the ask as typed, clamped to two lines with a fade; the replies row, left-aligned (`N replies, M new` in brand for unseen replies), the last reply's time at its right, the whole line a full-width "View thread" button on hover; the peek, a row of the stream with the Instrument mark carrying the state as a dot on its corner (brand working, amber waiting), then the running step in brand, the pending question, or the last reply's first words muted; and the assets as pills at full weight, absent when there are none. Sixteen pixels of air between threads and no hairline; the day heads stay centered. No avatar, no name, no last-reply text on the first line, and the reply count never changes meaning. A dashed "+ topic" chip slides in after the pills on hover and opens the topic list. Every way into the thread (the replies button, the ask, the peek) opens it at the same place, in the tab on screen; a topic pill filters the list to that topic; a hold opens its file, app, or site; the row's padding is not a door.
