...
3. Crop from the original, then scale the crop **up** to fill the budget, so each element covers as many patches as possible. Return JPEG, not PNG, because every crop stays in the transcript and full-budget ONGs blow the 32MB request limit.
...
11. Corners are ordered and clamped, and the rectangle actually used comes back in the result. An empty or fully out-of-bounds region answers the model rather than failing the turn. **Echoing the used rectangle turned out not to be enough on its own** -- see phase 5 -- so a clamped read now leads with the rectangle that was asked for and the space it overran.
...
19. **Restraint is where models differ, not capability.** Sonnet and Grok answered the legible image from the first look. The gpt-5.6 family zooms before it has seen anything: on every image read it opened with an all-zero rectangle, and on the legible case it went on to request a whole-image region it did not need. Adding "omit this on a first read" to the parameter description was tried and measured to produce _more_ of it. Both shapes are answered with the plain whole image now, on the reasoning in phase 5.
20. **The bounds error is load-bearing.** A model that guessed the wrong coordinate space recovered from the sentence naming the right one. That error text is a feature, which is also why a rectangle naming no place is answered rather than refused: it names no space to correct.
...
Three defects in the region read came out of the same runs, all of them the model filling in a parameter rather than aiming at anything: an all-zero rectangle on every image read, a 1x1 rectangle magnified into a flat color the model reported as a blank screen, and a whole-image region requested before it had seen the image. **The general lesson is that a magnified crop is believed**, so any rectangle that cannot carry a picture has to be refused rather than rendered.

### Phase 5: what real sessions do with it -- landed

The eval cases are single-question tasks aimed at images built to need a zoom. Phase 5 asks the opposite question of a corpus of ordinary sessions: **286 `read_file` calls that touched an image, across 71 tasks, 66 of them carrying a region.** Every conclusion below is from that sample, and it moved three of this plan's assumptions.

21. **Three of the 66 region reads were the feature being used as designed.** The rest break down as 43 naming no place (all-zero), 6 the unit square `(0,0)-(1,1)`, 7 covering the whole view, 6 aimed but in the wrong pixel space, and 1 empty. The parameter is filled in roughly twenty times for every time it is aimed, which is the ratio the tool now has to be built around rather than the one the eval cases suggest.
22. **The clamp was the expensive defect, not the refusal.** Five successful reads asked for a rectangle outside the view and were quietly trimmed to fit: one asked for the left half in the file's pixel space and was handed the full width, another asked for a square and got a letterbox. Each came back as a real magnified picture of somewhere the model did not ask about, described by a sentence naming the rectangle used -- which reads as confirmation. This is the same "a magnified crop is believed" lesson as the 1x1 flat color, and it is worse, because the erroring shapes cost a round trip while this one costs an answer. A clamped read now leads with the rectangle asked for and the space it overran.
23. **File-space coordinates are the dominant aiming error**, not arbitrary wrong numbers. Models read "3840x2160 px, shown to you at 1456x819" and take the first pair. Naming both sizes is still right -- the file's is what still exists on disk, and what a crop script would be written against -- so the fix is which pair leads. A read now says "shown to you at 1456x819 px; the file on disk is 3840x2160", putting the coordinate space first and framing the file's size as a property of the file rather than of the picture, and the downscale note names the space a rectangle's corners belong to outright.
24. **A whole-view rectangle can never be a zoom.** The crop renders to the same budget the preview did, so it carries no detail the first read did not, and for a file already inside the budget it is the same pixels interpolated larger: one observed read blew a 128x128 icon up to 1092x1092 and spent a full image budget on it. Answered with the plain preview now.
25. **This is not one model family's habit.** Not one region read came from a session pinned to a Claude model, across 108 image reads. The rest came from sessions on the automatic model and from glm-5.2, and the aiming errors in 22 and 23 are spread across families rather than concentrated in the one the eval cases flagged.
26. **Which model `auto` resolved to is not recorded**, so a session on the automatic model cannot be attributed after the fact. That is a gap in its own right and the reason 25 is stated as "not pinned to Claude" rather than a clean per-family split.

The tool now answers a rectangle naming no place and a rectangle covering everything with the ordinary whole-image read, and says which it did. Degrading silently was the earlier behavior for the all-zero case, and the corpus shows what that costs: the model cannot tell the parameter did nothing, and the reads that followed one asked for the whole image as a rectangle instead.

The two zoom cases carry a third assertion for the space a rectangle landed in: on 8000-wide fixtures shown at 1269, a rectangle aimed in the file's pixel space overruns the view and is trimmed, so a crop carrying `requestedRegion` is that error's signature. It is a guard rather than a demonstration -- every model measured aims in the shown space on these fixtures, and the wrong-space aim is something real sessions produce and this fixture set does not -- so what it protects is the wording in 23 against a later change that quietly moves models back. An ignored rectangle is deliberately not judged by it, on the same reasoning as `isRegionRead`: a parameter filled in rather than aimed says nothing about which space the model would have used, and counting it as a miss fails a model whose actual aim was exact.

Re-running phase 4's cases confirms it, and turned up one more shape while doing so. The negative case, red since it was written, passes on gpt-5.6: it opens with `(-1,-1)-(0,0)`, gets the whole image and a sentence saying why, and answers from that one look. Enumerating the shapes was never going to work -- **the test belongs on the clamped corners, not the ones that arrived**, which is what catches a variant nobody has seen yet. The two zoom cases still crop and still read their answers exactly, on gpt-5.6 and claude-sonnet-5 both, so nothing here costs a model that was aiming properly.

## Risks and open questions

- **Context cost.** Each crop is a full-budget image that lives in the transcript forever. The cookbook measured 10-30x per-question cost. In a long general-purpose session, three zooms across a task is fine; a model that zooms reflexively is not. The digest cache in phase 1 removes the repeated-encode cost but not the context growth. Neither reference harness solves this -- both hold every image in history. **Phase 5 settles it for now, and against a cap.** Across real sessions the cost was not runaway refinement but rectangles that were never zooms: whole-view reads and rectangles naming no place, which are answered from the preview now and so cost no second image at all. No session showed a model subdividing its way through a budget. A cap on crops per turn would therefore bite the one behavior worth keeping. Revisit if a session ever shows real refinement running long.
