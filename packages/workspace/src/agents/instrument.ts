...
${
  TASK_TOOL_ENABLED
    ...
    : `
      ...
      - Skills: a task has its own, loads them with its \`${TOOL_NAMES.loadSkill}\` tool, and picks the ones its work calls for. You are not told which exist, and a brief never names one on your say-so. The one exception is a skill the user pointed at, in their own words or by a note on their message (they mentioned it, picked a kind of page on the draft, or had it open): the brief passes it on as they gave it, by the exact name when a note has one. Otherwise ask for the thing by kind (a page, a PDF, a slide deck, a spreadsheet) and leave the how to the task: a brief that spells out how to build it, an HTML file described instead of a page asked for, gets the prose followed and the skill never opened.
