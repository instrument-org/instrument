...
**Watch the pane; poll the popover.** The toolbar's file list is a browsing surface, so it is fetched by the panel that draws it and re-fetched every five seconds while that panel is mounted. Radix unmounts a closed popover, which makes the scope free: open means polling, closed means nothing. A poll's cost scales with what is on screen, a watch's with the size of the tree, and these two surfaces sit on opposite sides of that. The split is deliberate rather than an inconsistency to tidy up later.

The poll is cheap because the walk prunes rather than traverses: `isIgnoredTaskPath` is checked before descending, so `node_modules` costs one directory entry and not a subtree. Measured warm across a 596-task workspace, one walk runs a median of 0.21ms, p99 1.8ms, and 8.3ms for the largest task in it (418 files, a cloned repo). Every five seconds, while a panel is open, that is not a number worth optimizing.
...
3. ~~**The pane subscription** (here). `fs.watchFile` on the open path, owned by the file tab.~~ **Landed** as `files.live.info` and [watch-file-info.ts](../../../packages/workspace/src/lib/watch-file-info.ts). It is the only live thing left in the app.
4. ~~**Delete `data-fileChanges`** (here).~~ **Landed.** Adherence was re-measured first: 45 of 52 assertions passed, and the four production-shaped cases passed on every model, so the fence carries what the card used to.
5. ~~**The file-list popover to a one-shot read**, then delete the watcher, `files.live.list`, and `CurrentTaskFilesProvider`.~~ **Landed**, and further than written. The watcher and the standing index went first, replaced by a walk re-emitted at turn end; that re-emit then went too, because turn boundaries are when we look rather than when files change. The popover fetches for itself and polls while open, and the Files button became unconditional, which is what removed the last reader of the list from outside the panel.
...
- **The popover is up to five seconds stale**, and blind while closed. Both are the point: nobody can perceive the first, and the second is what a poll buys over a watch. If it ever reads as broken, the answer is a shorter interval, not a standing index.
