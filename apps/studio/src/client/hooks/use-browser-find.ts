...
export function useBrowserFind({
  ...
  // should reach. The find opener is a single slot: without this, a host that
  // parks its guest keeps claiming it, and an overlay that takes the slot and
  // clears it on unmount leaves the panel -- whose own inputs never changed --
  // never re-registering, so Cmd+F stops working entirely.
  covered?: boolean;
  isActiveTab: boolean;
