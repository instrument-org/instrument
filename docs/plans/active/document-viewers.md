...
So those two formats bind Cmd/Ctrl+C inside the viewer instead, guarded on that viewer actually holding a selection: an unguarded copy emits an empty string, which would wipe the clipboard on any copy elsewhere in the app, and both the artifact panel and the expand modal can have a viewer mounted at once.
