...
- Every window (see Windows) uses the same `ZoomRoot` + `zoomAtom`: the onboarding and orchestrator windows wire it via `OnboardingZoomRoot`. `ZoomToast` (a transient corner readout on any zoom change) is mounted once per window, outside `ZoomRoot`, so keep it in sync in both roots. Each root also calls `useSyncZoom`, which is what re-centers a window's macOS traffic lights: they are real pixels drawn over a band of chrome whose height is the zoom the renderer draws it at (`windows/traffic-lights.ts`).
