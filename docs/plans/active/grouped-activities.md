...
[frames.test.ts](../../../apps/studio/src/client/routes/_app/debug/-transcript/frames.test.ts) holds the fold to account where it is decided rather than where it is drawn: one call running at a time in every scenario, a part never preceding its step, an error landing on the step it happened in. How it looks is still the page's job, and what reaches the DOM is the job of the component tests around `chat-stream`.

A render sweep over the library sat beside it for a while, drawing all 942 frames and asserting on the classes that came out. It went: mutation testing put four real regressions through it -- a leaking fold, an empty group box, a lost live gate, a duplicated in-flight row -- and it caught one, which those component tests caught too. Two of its three rules could not fail at all by then. It cost forty times what the file above costs.

Membership is decided in [transcript-layout.ts](../../../apps/studio/src/client/components/transcript-layout.ts), in the pass that already walks the parts in order for run adjacency. Nothing in the data says a call belongs to a group: no tool part carries a group id, and a declared group is only ever implied by the agent having announced one first. So membership is read positionally, and a group opens at a heading or at the first step after a break and closes at the next heading or at the end of the turn.
...
**Opening a step opens the phase around it, and leaves it open.** The fold draws one line for a whole phase and the next step takes that line, so a step opened inside a folded group is replaced by whatever the agent did next -- the reader clicks, reads two words, and what they opened is gone. Opening the phase gives it somewhere to be: it moves out of the group's slot and into its place in the run, with the steps either side of it, and stays there while the phase works past it. Which means a row cannot hold its own open state, since the copy in the slot and the row in the run are two different elements: the transcript holds it, keyed by part, and `useRowExpansion` in [transcript-expansion.ts](../../../apps/studio/src/client/components/transcript-expansion.ts) is what a row reads it through.

**One call opens itself: image generation.** Every other call is quick enough that the line saying it started is the whole of what there is to see, or textual enough that its row already says what it did. This one runs for the better part of a minute and produces a picture, so the transcript opens it the first time it catches it running (`opensOnSight`), and the phase around it with it, which is what leaves the picture on screen once the run has moved on. Taken once per call, so a reader who shuts it stays shut, and only while it is live: a task reopened afterwards is a list of the phases it went through, the same as any other.

**One 8px rhythm, top to bottom.** Every container in the transcript spaces its children by `gap-2`. A step row cannot: it carries 4px of its own padding so there is something to click, which would put its neighbors 16px apart. So the group box stacks its rows flush and pulls itself in by that padding (`-my-1`), which leaves the text of a step exactly 8px from whatever is above and below it -- another step, a head line, or a paragraph of prose. No row anywhere in the transcript carries a vertical margin of its own: rows that do move the whole block every time the fold changes which of them is showing, and a run turning into a group would change how tall it is as well as what it says.
...
The whole fold is `planRow` in [transcript-layout.ts](../../../apps/studio/src/client/components/transcript-layout.ts), fed by `buildTranscriptLayout`; both are pure over the parts, and [transcript-layout.test.tsx](../../../apps/studio/src/client/components/transcript-layout.test.tsx) snapshots each case as the shape it produces, so a change to any rule shows up as a change to the picture.
