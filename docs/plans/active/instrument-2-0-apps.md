...
The afternoon, from the user's own use. PostHog connected from a plain sign-in button. Cloudflare's whole-API MCP server refused the client the app had just registered ("Invalid client_id"), so the directory now says which of its servers take a client registered on the spot and which need one Cloudflare has issued. The sign-in callback landed on a page of ours about an unrelated sign-in with an "Open Instrument" button, in a tab inside Instrument, so a sign-in that ran in the window's browser now lands on the service's own signed-in home, and one that ran in the user's browser gets a page naming the app and the way back. The app's page kept its actions at the bottom and grew when its tools arrived, so they are a menu by the name, the service's home is the primary action beside the ask, and placeholder rows hold the height. A card from an earlier session whose app had been removed offered a sign-in to nothing and failed on the missing folder, so it says the app was removed. And the mark was on sign-in and key buttons that send the agent nothing, so it is only on buttons that do, and the user's own browser is a plain second button rather than a menu behind a chevron.

Some services have no server on the internet at all: an app on this Mac, or a tool that reads the user's own files. Those ship an MCP server that runs here, over stdio, and a third manifest type carries one: `mcp-local`, naming a package and a runtime rather than a URL. A node server is installed once from npm into a workspace-managed folder and started on the node the app is already running, by absolute path, so nothing depends on a `node` being on PATH or on a shebang resolving; a python one goes through uv, which brings its own interpreter. The manifest can name a package and never a command, and the package spec is validated against the shape its registry uses, so a folder the agent writes cannot name a binary on this machine or smuggle an argument into the install. The agent could already spawn a process through the sandbox's real-binary escape hatches, so this is not a new capability; what it would have been is a quiet one, and the card is the answer. The user allows a package before it runs at all, the approval is pinned to the manifest hash the way a connection is, and changing the package asks again.
