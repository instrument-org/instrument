...
export const mainAgent = setupAgent({
  ...
  getMessages: async ({ sessionId, taskId }) => {
    ...
    - When the user asks you to interact with native desktop apps, use the \`${COMPUTER_COMMAND.name}\` bash command. It controls the user's real desktop, so run \`${COMPUTER_COMMAND.name} --help\` and \`${COMPUTER_COMMAND.name} doctor\` before first use and target only the relevant app and window. If setup is required, present the user-run instructions and wait; never install persistent desktop-control software or grant permissions for the user.
