...
The patch edits one minified chunk of `dist/bundle/`, so a version bump drops it and it has to be re-derived from the PR's source diff by hand: find the `readdirWithFileTypes` call in the chunk that holds `find`, and wrap the two `readdir` calls the way `find.ts` does on the branch. `pnpm patch just-bash@<version>` opens the copy to edit and `pnpm patch-commit` writes it back.
