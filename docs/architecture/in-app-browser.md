...
Downloads split the same way in [`downloads.ts`](../../apps/studio/src/electron-main/browser-view/downloads.ts). Every guest shares the one workspace browser session, so its single `will-download` listener attributes each download to the guest that started it. A guest the agent authorized through `Browser.setDownloadBehavior` (what agent-browser's `download` command sends) saves under the GUID that command waits for, and its completion is synthesized back as `Page.downloadProgress`. Any other download is the person's own click: a `<webview>` shows nothing of its own for one, so it lands in the task's `downloads/` folder under its own name, suffixed rather than overwritten, and the window hosting the guest gets a `browser.download-finished` event to say so ([`browser-download-notices.ts`](../../apps/studio/src/client/lib/browser-download-notices.ts)). Only a download from a guest no entry owns is canceled.
