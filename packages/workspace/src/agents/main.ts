...
export const mainAgent = setupAgent({
  ...
  getMessages: async ({ appConfig, sessionId }) => {
    ...
    - IMPORTANT: You do not automatically see files written to disk. To inspect any image or media you create, read it back with \`${agentTools.ReadFile.name}\`. When the user specifies visual criteria (composition, margins, style) or provides a reference image, always read your output before reporting done -- do not assume the command produced correct results. If the user provided a reference, read both files to compare them visually.
