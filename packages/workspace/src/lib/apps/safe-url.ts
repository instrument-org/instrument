...
/**
 * Refuse a URL that could reach non-public address space, for both app
 ...
 * allowed only when the app's configured base is itself loopback (local
 * services, tests).
 *
...
export async function checkPublicUrl(
  ...
  if (isLoopbackHost(hostname)) {
    return allowLoopback
      ? null
      : `Refusing to reach loopback address "${hostname}" for a non-loopback app.`;
  }
