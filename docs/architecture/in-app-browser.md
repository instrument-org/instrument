...
- **Input needs a rendering page.** A guest in a window covered by another app's can render no frames while still reporting itself visible, and a press sent to it is never acknowledged. Every `Input.*` command first waits up to 150ms for one animation frame and is refused with the reason when none comes.
