...
| PR | what | affects us |
| --- | --- | --- |
| #420 | `stat`: expand every `-c` directive, and print `?` for one that cannot be answered | Yes, and it is patched locally. Every directive outside the implemented ten, the timestamps included, came back as its own source text at exit 0; see the patch above. |
