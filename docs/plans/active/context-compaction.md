...
- **Rollover carries session context, the retained user messages, and the agent's handoff notes inlined into the request.** The full state block phase 2 describes, in grok-build's `reminder.rs` shape, is not built: no working folder, attached folders, or file lists. The notes are the part of it that was load-bearing, and they are mechanically re-stated rather than pointed at, which is that section's actual lesson.

  This half was built twice. The first version warned the agent to write notes and, after a rollover, told it where they were and to go and read them. That does not work, and the failure is worth keeping because it is not obvious: told its window was nearly full, an agent wrote correct notes naming the exact format it had been asked for, rolled over, and then answered three more turns in a format that had lost that detail, with the notes sitting unread on disk the whole time. Re-run with the notes planted and the instruction present, it still declined to spend the tool call. A user message asking for something else is right there, and an unnamed file behind a tool call loses to it every time.

  What the plan already said about this, under "Decisions to make before building", turned out to be the whole of it: *"Notes need somewhere durable to live [...] the write-your-notes prompt should name the path. This is the load-bearing dependency of the cheap phases and it is currently unspecified."* It is now specified, in one constant that both the warning and the reader derive from, so the writer and the reader cannot disagree about where the notes are. Verified against a real agent: three consecutive rollovers in one session, format preserved across all of them, with the agent keeping its own notes current in between.
...
4. **Triggers.** Pre-turn, post-turn, on a classified context-overflow error, and on a switch to a model with a smaller window. Never on user interruption, for the reason above; a cancelled turn leaves the history alone.

   **The reactive one is built.** [context-overflow-reported.ts](../../../packages/workspace/src/lib/context-overflow-reported.ts) reads the `context-overflow` verdict `classifyProviderError` already records on the failed turn, and a refusal rolls the session over whatever the arithmetic made of the window. It came in with the model-switch work below rather than on its own schedule, because a budget that declines to act on a count it does not trust needs something that acts on a count nobody has: without it a history too large for the new model is sent, refused, and sent again unchanged forever.

   **The switch to a smaller window is handled by deferral rather than compaction.** A count from the previous model warns the agent but does not reset the session, so the warning that produces handoff notes arrives before the reset instead of being skipped by it, and the current model's own first count decides what actually happens. Compacting on the switch, using the previous model to do it, remains the codex shape and remains unbuilt; it is what would close the one turn this leaves exposed, where a history the previous model filled goes to a smaller model once and is refused.
