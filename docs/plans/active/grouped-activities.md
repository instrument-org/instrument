...
**How it gets looked at: the playback page**, `/debug/components/playback`. A scenario is a list of the things the agent did ([script.ts](../../../apps/studio/src/client/routes/_app/debug/-playback/script.ts)); `buildFrames` folds it into every state the transcript passes through on the way ([frames.ts](../../../apps/studio/src/client/routes/_app/debug/-playback/frames.ts)); the page scrubs, plays, and single-steps that list through the real chat stream in the real scroller. Because a frame is the fold of the first `n` events and nothing else, dragging, playing, and stepping cannot disagree, and there is no timer in the model at all. Parts keep the id they were born with across their states, so a call is one row changing rather than a row replaced -- which is the difference the rendering rules turn on.
...
**Prose is the one boundary that reads differently for the two kinds.** It closes an inferred group, because an unannounced run is only a run and the agent turning to address the user is the clearest break in one. It does not close a declared group: there the agent has said where the boundary is, so a note dropped mid-phase belongs to the phase it interrupted, folds away with it, and comes back when the reader opens it.

**A paragraph is held by its phase but never indented under it, and a turn's last one leaves.** Nothing closes an activity from the inside, so a turn that used tools always ends inside one, and that is where the answer the user came for gets written. Folding that phase would take the answer with it, so once the turn is over its last paragraph belongs to no phase. Whether a paragraph is the answer or a note the agent is about to work past can only be told from what comes after it, and while it is arriving there is no after -- so it is told at the end of the turn, which is the first moment it is decidable. Left at the margin all along, being told costs the paragraph no movement: it is in the same place before and after, and all that changes is whether the fold can still reach it. This is also why the answer is read as the last thing a turn *said* rather than the last row it produced: a note the run files afterwards -- a project change, a file the watcher saw, whatever developer mode is drawing -- says nothing about whether the paragraph above it was the answer. Every turn keeps its own, so sending another message does not fold away what the last one was told.
...
**No end call.** An activity is closed by the next one starting or by the turn ending. An explicit end doubles the calls and adds a way to get it wrong, and the final message already says how the work came out. It would also be a rule the model can forget, so the turn-ending fallback has to exist regardless -- and a forgotten end fails by folding the answer away, which is the one outcome worth designing against. What it would buy is the answer being outside a phase from its first token rather than from the end of the turn, and since prose sits at the margin either way, that difference is not visible.
...
- A declared group that prose no longer closes runs to the end of the turn. An agent that declares one activity and then works for forty calls now gets one group with a title from the first minute of it. Collapsed that is one stale line rather than forty unexplained rows, so it is an improvement either way, but it is the cadence gap showing up somewhere new.
