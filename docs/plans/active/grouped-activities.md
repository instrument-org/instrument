...
[frames-render.tsx](../../../apps/studio/src/client/routes/_app/debug/-transcript/frames-render.tsx) draws every frame of every scenario and checks what has to hold in all of them: at most one row marked as the work in flight, no empty group box, and nothing indented until a group is opened. That is the part of watching it a machine can do; how it looks is still the page's job. It is expensive enough to be the last thing running in its Vitest project, so the sweep is split across the `frames-render-N.test.tsx` files that call it.
