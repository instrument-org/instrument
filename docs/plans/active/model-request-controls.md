...
**What the level is for calls the user did not make.** Title generation runs a model the user picked for a different job, to produce at most eight words from a budget reasoning shares, so it carries its own low level rather than inheriting whatever the task is set to. That is true whichever of the four above wins, because none of them is a control over it.

The search model is the near miss worth naming, because it looks like the same case. It is also a model picked for something else, but it runs the searches itself and its prompt asks it to attribute each claim, to report disagreement between sources rather than resolve it, and to catch a misheard proper noun. Lowering its level spends exactly that. It keeps the provider default until something measures the trade.
