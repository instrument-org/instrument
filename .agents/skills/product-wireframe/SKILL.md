...
The `text/tailwindcss` block opens with no `@import`, and nothing inside it may spell that at-rule, comments included. Chrome's preload scanner reads inline style text for imports before it honors the element's type, so an `@import "tailwindcss"` there is fetched relative to the page and answers 404 in every viewer's console; the browser build adds the import itself when a substring search of the block finds none, and finds one in a comment just as well.
...
**The click before, the new after.** In a sequence the reader has to see what was clicked to get from one frame to the next and what changed because of it, and neither may look like part of the design. A click mark goes on the frame **before** the result, on the thing in its pre-click state: the row before it is lifted, the button before the draft it opens, the tab before the page it brings. The next frame is the result, and it marks what changed with a new mark. Never put a click mark on a frame that already shows what the click produced; the reader then sees a click and its consequence at once and cannot tell which is which, which is the single most common way these files go wrong. A frame that is only a state (day one, a week in, the narrow window) carries no marks at all. One click per frame; new marks as many as things changed, but two is usually the ceiling.

The helpers: `clickable(inner)` wraps a block (a row, a card, a button, a sidebar entry) as the thing about to be clicked; `fresh(inner)` wraps what appeared or changed; `ann("click")` or `ann("new", "new")` dropped inside any element marks that element without a wrapper, and `cursor` dropped inside marks it as the click. None of them needs the element to be `relative`, and none of them can be clipped: the helpers leave an invisible marker, and after render the page measures each marked element and draws its ring and tag on a layer over the whole frame, with the tag above the ring when there is room and below it when there is not. Orange with a pointer glyph in the tag is the click; violet is the new. The colors are ones the product never uses, so they read as commentary on the frame. Never a green row, a highlighted tile, or a check mark standing in for either: those look like the product, and they are not.
...
| Brand marks           | A third party's logo (a model provider, a service the frame names) is fetched while writing, checked, and inlined as SVG or a data URI. Never a runtime URL into an icon CDN: those drop brands on request, and a slug that is gone is a 404 in every viewer's console for the life of the file, where a fetch that fails while writing is caught by whoever is writing |
